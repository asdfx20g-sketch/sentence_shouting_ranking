...
export default function App() {
  ...
  const pendingTimeRef = useRef(null); // 타이머 정지 후 저장 전 임시 보관
  ...
  // 저장 버튼 누를 때: pendingTime 포함해서 한꺼번에 저장
  function handleSave() {
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {Object.entries(classes).map(([cls, students]) => {
            ...
            return (
              <button key={cls} onClick={() => { setSelectedClass(cls); setSelectedStudent(null); setView("scoring"); }}
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-rose-50 via-amber-50 to-rose-50 p-6">
        ...
        <div className="max-w-2xl mx-auto pb-10">
          ...
          {/* 타이머 — 체크박스 위에 배치 */}
          <TimerBlock
            recordedTime={score.recordedTime}
            onTimeUpdate={(t) => { pendingTimeRef.current = t; }}
          />
  ...
  return (
    <div className={`rounded-2xl shadow-sm border p-4 mb-4 ${isOver3min && running ? "bg-red-50 border-red-200" : "bg-white border-rose-100"}`}>
      <div className="flex items-center justify-between mb-3">
        ...
        {recordedTime && (
          <span className="text-xs bg-rose-100 text-rose-700 font-semibold px-2.5 py-1 rounded-full">저장된 기록: {recordedTime}</span>
        )}
      </div>
      <div className="flex items-center gap-4">
        <div className={`text-4xl font-bold tabular-nums ${isOver3min && running ? "text-red-600" : "text-rose-900"}`}>
          {formatTime(seconds)}
        </div>
        <div className="flex gap-2 items-center">
          {!running ? (
            ...
            <button onClick={handleStop} className="bg-rose-600 hover:bg-rose-700 text-white font-semibold px-4 py-2 rounded-xl text-sm transition-colors">
              정지
            </button>
          )}
          {stoppedTime && !running && (
            <span className="text-sm text-emerald-700 font-semibold bg-emerald-50 px-3 py-1.5 rounded-xl">
              ✓ {stoppedTime} — 저장하기 누르면 기록됩니다
            </span>
          )}
        </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-rose-50 via-amber-50 to-rose-50 p-6">
      <div className="max-w-2xl mx-auto pb-10">
        ...
        <div className="space-y-4">
                ...
                {editingClass === cls ? (
                  ...
                  <>
                    ...
                    <div className="flex items-center gap-1">
                      <button onClick={() => startEditClass(cls)} className="p-1.5 text-gray-400 hover:text-rose-700 transition-colors"><Pencil size={15} /></button>
                      <button onClick={() => { if (window.confirm(`"${cls}" 반을 삭제할까요?`)) actions.deleteClass(cls); }} className="p-1.5 text-gray-400 hover:text-red-600 transition-colors"><Trash2 size={15} /></button>
                    </div>
                  </>
                )}
              </div>
          ...
          {Object.keys(classes).length === 0 && <div className="text-center text-gray-400 text-sm bg-white rounded-2xl p-8 border border-rose-100">등록된 반이 없어요. 위에서 반을 추가해주세요.</div>}
        </div>
      </div>
    </div>
  );
